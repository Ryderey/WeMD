...
### 超长块兜底
- 单原子块高 > 页可用高度 → 标记超长块；预览区黄色警告条显示数量与类型。
- 点击导出时弹确认框：「N 个块超出一页高度，将等比缩小适配到单页，是否继续？」确认 → 该块整体 `transform: scale()` 缩小居中放入单页；取消 → 回到对话框。
...
## 4. 实时预览
- 设置变更 debounce 300ms → 重建容器 → 切分 → 低清缩略图（低 pixelRatio 截图）刷新预览；更新期间 loading 态。
- 预览区展示前 3 页缩略图 + 「+N」占位；点击缩略图打开 lightbox 看大图；顶部标注「共 N 页 · W×H」。
