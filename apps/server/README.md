...
桌面端（开发模式）启动时会自动拉起本服务并监听 14000 端口，无需手动执行下述
命令；手动启动仅用于单独调试服务端，此时默认监听 4000 端口。
...
桌面端（开发模式与打包版）会自动拉起本服务：开发模式从源码启动并监听 14000
端口；打包版随安装包携带构建产物与生产依赖，启动时同样监听 14000 端口，微信
图床凭据从用户数据目录的 `server.env`（Windows 上为 `%APPDATA%\WeMD\server.env`，
每行 `KEY=VALUE`，字段同 `.env.example`）读取。若启动时 14000 端口已有服务在
监听，则直接复用现有服务不重复拉起。手动单独启动时默认监听 4000 端口。

除桌面内嵌方式外，当前只提供代码接口和本地启动方式。Docker/Compose、生产反向
代理、HTTPS 和固定出口 IP 部署尚未实现；若自行部署独立服务，需妥善保护环境变量，
并把服务的固定出口 IP 加入微信公众号后台的 IP 白名单。
