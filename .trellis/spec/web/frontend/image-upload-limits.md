...
| ------------------------------------- | ----------------- | --------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `wechat`                              | `950,000` bytes   | `>=` rejects (strictly less than) | Upstream `uploadimg` judges "less than 1M" in **decimal** bytes (1,000,000), while our uploader and server used to compare against `1024 * 1024`. The 1,000,000–1,048,575 window passed locally and failed at WeChat with an opaque `40009`. The remaining margin also covers multipart framing and the default `client_max_body_size 1m` (1,048,576 bytes of the **whole request body**, not just the file) when deployed behind nginx. |
