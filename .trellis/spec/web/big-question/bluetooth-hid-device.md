...
if (potentialRemotes.length > 0) {
  ...
  potentialRemotes.forEach((d, i) => {
    ...
    console.log(
      `    Vendor ID:  0x${d.vendorId.toString(16).padStart(4, "0")} (${d.vendorId})`,
    );
    console.log(
      `    Product ID: 0x${d.productId.toString(16).padStart(4, "0")} (${d.productId})`,
    );
...
function handleButtonPress(): void {
  ...
  if (
    timeSinceLastPress < CONFIG.doubleClickMs &&
